...
class ScriptArguments:
    ...
    data_path: Optional[str] = field(
        default="jbrophy123/medical_dataset", metadata={"help": "path for the dataset hosted on HuggingFace"} ## HuggingFace Data path
    )
    ...
    per_device_train_batch_size: Optional[int] = field(
        default = 4, metadata = {"help":"Specifies the number of samples to process in a single batch per device when training the model"} ## How much data to process per training batch
    )

    gradient_accumulation_steps: Optional[int] = field(
        default = 8, metadata = {"help":"Number of gradients to accumulate before taking the next step in the optimizer"} ## Accumulate gradients over several batches, and then performs an update
    )
    ...
    save_steps: Optional[int] = field(
        default = 250, metadata = {"help":"Number of updates steps before two checkpoint saves"} ## Number of steps before saving
    )

    logging_steps: Optional[int] = field(
        default = 100, metadata = {"help":"Number of update steps between two logs"} ## Number of steps before creating logs
    )
    ...
    num_train_epochs: Optional[int] = field (
        default = 3, metadata = {"help":"Number of times the entire training set is passed through the model"} ## Number of times model is passed through model for training, low number could lead to under fitting high number to over fitting
    ...
    lora_dir: Optional[str] = field(
        default = "a7pute/llm_medicalQA", metadata = {"help":"Directory to save the model"}) ## Save the model and outputs to hugging face account
